...
def ssim(a: np.array, b: np.array, **kwargs) -> float:
    ...
    return skimage.metrics.structural_similarity(im1=a, im2=b, win_size=kwargs.get("win_size", None))
...
def lipschitz_constant(a: np.array, b: np.array, c: np.array, d: np.array, **kwargs) -> tf.Tensor:
    ...
    if np.shape(a) == ():
        ...
        return float(kwargs.get("norm", distance_euclidean)(a-b) / kwargs.get("norm", distance_euclidean)(c, d))
