...
class FaithfulnessTest(Measure):
    ...
    def fit(
        ...
        assert (
            targets is not None
        ), "For each input, specify the target class you wish to explain against."
        ...
        for x, y, a in (inputs, targets, attributions):
            ...
            input_segmentations = segmentation_fn(
                x, a_ix, self.mask_value, self.mask_strategy, self.mask_order
            )
