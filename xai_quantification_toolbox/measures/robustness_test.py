...
class RobustnessTest(Measure):
    ...
    def __init__(self, **kwargs):
        ...
        ), "Specify a 'perturbation_function' that exist."
        assert (
            self.similarity_function in SIMILARITY_FUNCTIONS
        ), "Specify a 'similarity_function' that exist."
    ...
    def __call__(
        ...
        **kwargs
        ...
        assert np.shape(inputs) == np.shape(
            attributions
        ), "Inputs and attributions should have same shape."
        ...
        attributions = np.reshape(
            attributions,
            (-1, kwargs.get("img_size", 224), kwargs.get("img_size", 224), 1),
        )
        ...
        for index, (input, target) in enumerate(zip(inputs, targets)):
            ...
            # Generate explanation based on perturbed input and store its similarity to.
            input_perturbed = self.perturbation_function(input)
            attribution = attributions[index]
            attribution_perturbed = model.attribute(
                batch=input_perturbed,
                neuron_selection=target,
                xai_method=kwargs.get("xai_method", "gradient"),
            )
            results.append(
                self.similarity_function(
                    a=attribution, b=attribution_perturbed, c=input, d=input_perturbed
                )
            )
