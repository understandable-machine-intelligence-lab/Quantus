...
def test_model_parameter_randomsiation(data: dict, params: dict, expected: Union[float, dict]):
    scores = ModelParameterRandomisation(**params)(model=None,
...
def test_non_random_logit(data: dict, params: dict, expected: Union[float, dict]):
    scores = RandomLogit(**params)(model=model,
                                   x_batch=data["x_batch"],
                                   y_batch=data["y_batch"],
                                   a_batch=data["a_batch"])
    if isinstance(expected, float):
