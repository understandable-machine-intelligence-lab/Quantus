...
from quantus.nlp import AvgSensitivity, MaxSensitivity, uniform_noise
...
def test_avg_sensitivity_keras_model(tf_sst2_model, sst2_dataset, init_kwargs, call_kwargs):
    metric = AvgSensitivity(nr_samples=5, **init_kwargs)
    result = metric(tf_sst2_model, sst2_dataset, **call_kwargs)
    assert isinstance(result, np.ndarray)
    assert not (result == np.NINF).any()
...
def test_max_sensitivity_torch_emotion_model(emotion_model, emotion_dataset, init_kwargs, call_kwargs):
