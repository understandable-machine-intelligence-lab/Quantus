...
def test_mosaic_func(
    ...
    for mosaic, mosaic_indices in zip(all_mosaics, mosaic_indices_list):
        ...
        assert np.all(np.equal(mosaic[:, :width, :height], x_batch[mosaic_indices[0]])), "Test failed."
        assert np.all(np.equal(mosaic[:, width:, :height], x_batch[mosaic_indices[1]])), "Test failed."
        assert np.all(np.equal(mosaic[:, :width, height:], x_batch[mosaic_indices[2]])), "Test failed."
        assert np.all(np.equal(mosaic[:, width:, height:], x_batch[mosaic_indices[3]])), "Test failed."
