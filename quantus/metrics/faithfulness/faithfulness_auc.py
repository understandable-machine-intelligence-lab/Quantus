...
class FaithfulnessAUC(PerturbationMetric):
    """
    ...
    def evaluate_instance(
        ...
        for i_ix in range(self.nr_runs):
            ...
            y_pred_current= float(model.predict(x_input)[:, y])
            areas.append(float(0.5*(y_previous_pred+y_pred_current)))

            # Sum attributions of the random subset.

        auc_score = np.sum(areas)
