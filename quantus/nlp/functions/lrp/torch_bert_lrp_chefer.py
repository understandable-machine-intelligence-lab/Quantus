...
def get_activation(activation_string):
    if activation_string in activation_to_function():
        return activation_to_function()[activation_string]
    else:
        raise KeyError(
            "function {} not found in ACT2FN mapping {}".format(
                activation_string, list(activation_to_function().keys())
            )
        )
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        if isinstance(config.hidden_act, str):
            self.intermediate_act_fn = activation_to_function()[config.hidden_act]()
        else:
