...
def list_parameterizable_layers(model: tf.keras.Model, flatten_layers: bool) -> List[tf.keras.layers.Layer]:
    if flatten_layers:
...
def get_random_layer_generator(
    model: tf.keras.Model, order: str = "top_down", seed: int = 42, flatten_layers: bool=False
    ...
    for layer in layers:
        ...
        model.get_layer(layer.name).set_weights([np.random.permutation(w) for w in weights])
...
def random_layer_generator_length(model: tf.keras.Model, flatten_layers: bool=False) -> int:
