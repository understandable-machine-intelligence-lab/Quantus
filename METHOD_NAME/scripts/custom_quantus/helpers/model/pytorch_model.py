...
class PyTorchModel(ModelInterface):
    ...
    def get_randomisable_layer_names(self, order: str = "top_down"):
        """
        Returns the randomizable layer names of the model in the correct order
        ...
        -------
        layers, List
            the randomizable layer names of the model
        """
        layer_names = [l[0] for l in self.model.named_modules() if (hasattr(l[1], "reset_parameters"))]
        if order == "top_down":
    ...
    def perturb_model_weights(self, model, std: float = 0.1):
        """
        """
        for param in model.parameters():
            param.data *= torch.normal(torch.ones_like(param.data), std)


    def get_random_model_until_layer(self, order: str = "top_down", layer_name: str = ""):
        """
        Instead of generating in a cascading manner, generates the whole randomized model until a specified layer.
        ...
        order: string
            The various ways that a model's weights of a layer can be randomised.
        layer_name: string
            name of the last layer to be randomized. Set to "" to randomize the whole model
        ...
        for module in modules:
            if order == "independent":
                random_layer_model.load_state_dict(original_parameters)
            module[1].reset_parameters()
            if module[0] == layer_name:
                break

        return module[0], random_layer_model
